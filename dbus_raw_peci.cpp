...
int main()
{
    try
    {
        ...
        std::shared_ptr<sdbusplus::asio::dbus_interface> ifaceRawPeci =
            server->add_interface("/com/intel/peci",
        ...
        ifaceRawPeci->register_method(
            "Send", [](const std::string& peciDev,
                ...
                // D-Bus will time out after too long, so set a deadline for
                // when to abort the PECI commands (at 25s, it mostly times out,
                // at 24s it doesn't, so use 23s to be safe)
                ...
                for (size_t i = 0; i < rawCmds.size(); i++)
                {
                    ...
                    // If the commands are taking too long, return early to
                    // avoid a D-Bus timeout
